...
import com.blueline.net.sms.codec.cmpp.packet.CmppDeliverRequest;
import com.blueline.net.sms.codec.cmpp.packet.CmppReportRequest;
...
import com.blueline.net.sms.common.util.FstObjectSerializeUtil;
...
import static com.blueline.net.sms.common.util.DefaultMsgIdUtil.bytes2MsgId;
import static com.blueline.net.sms.common.util.DefaultMsgIdUtil.msgId2Bytes;
...
public class Cmpp7FDeliverRequestMessageCodec extends MessageToMessageCodec<Message, CmppDeliverRequestMessage> {
	...
	protected void decode(ChannelHandlerContext ctx, Message msg, List<Object> out) throws Exception {
		...
		requestMessage.setMsgId(bytes2MsgId(toArray(bodyBuffer,CmppDeliverRequest.MSGID.getLength())));
		...
		if (requestMessage.getRegisteredDelivery() == 0) {
			...
			requestMessage.getReportRequestMessage().setMsgId(bytes2MsgId(toArray(bodyBuffer,CmppReportRequest.MSGID.getLength())));
	...
	protected void encode(ChannelHandlerContext ctx, CmppDeliverRequestMessage requestMessage, List<Object> out) throws Exception {
			...
			bodyBuffer.writeBytes(msgId2Bytes(requestMessage.getMsgId()));
			...
			if (!requestMessage.isReport()) {
				...
				bodyBuffer.writeBytes(msgId2Bytes(requestMessage.getReportRequestMessage().getMsgId()));
