...
public class MsgId implements Serializable {
    ...
    public MsgId(long timeMillis, int gateId, int sequenceId) {
        ...
        this(DateFormatUtils.format(timeMillis, "MMddHHmmss"),
                gateId,
                sequenceId);
    }

    public MsgId(String date, int gateId, int sequenceId) {
        setMonth(Integer.parseInt(date.substring(0, 2)));
        setDay(Integer.parseInt(date.substring(2, 4)));
        setHour(Integer.parseInt(date.substring(4, 6)));
        setMinutes(Integer.parseInt(date.substring(6, 8)));
        setSeconds(Integer.parseInt(date.substring(8, 10)));
        setGateId(gateId);
